...
class TestCreateStructure(unittest.TestCase):
    def test_accepted_blue_stone_onboard(self):
        ...
        self.assertIsNotNone(
            ...
            "Should allow creation of correctly positioned, recognized object",
        )

    def test_reject_coordinates_out_of_board_lower(self):

        with self.assertRaises(
            ValueError, msg="Should not accept coordinates outside of the board"
        ):
            Structure("blue", "stone", -1, -1)
    ...
    def test_reject_unrecognized_colors(self):
        ...
        with self.assertRaises(
            ValueError, msg="Unrecognized colors (green, blue, white, black) are not to be accepted"
        ):
